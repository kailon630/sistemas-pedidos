...
const AttachmentsSection: React.FC<Props> = ({ requestId }) => {
  ...
  // 1️⃣ Carrega a lista de anexos
  const loadAttachments = () => {
    api
      .get<Attachment[]>(`/requests/${requestId}/attachments`)
      .then((res) => setAttachments(res.data))
      .catch(() => alert('Erro ao carregar anexos'))
  }

  useEffect(loadAttachments, [requestId])

  // 2️⃣ Seleção de arquivo
  ...
  // 3️⃣ Upload via multipart/form-data
  const handleUpload = async (e: FormEvent) => {
    ...
    try {
      ...
      setFile(undefined)
      loadAttachments() // recarrega após upload
    } catch {
